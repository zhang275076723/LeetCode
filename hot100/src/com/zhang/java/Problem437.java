...
public class Problem437 {
    ...
    /**
     * 路径前缀和，遍历每个节点，在哈希表中查找前缀和为targetSum-curSum的节点个数
     * 哈希表：key，根节点到当前节点路径上，除当前节点之外，所有节点到当前节点的路径之和；value，满足路径前缀和的个数
     * 时间复杂度O(n)，空间复杂度O(n)
     *
     * @param root
     * @param targetSum
     ...
     * @param targetSum 要求的路径之和
     * @param curSum    当前路径之和
     * @param map       根节点到当前节点路径上，除当前节点之外，所有节点到当前节点的路径之和哈希表
     */
    private void dfs2(TreeNode root, int targetSum, int curSum, Map<Integer, Integer> map) {
        ...
        int curCount = map.getOrDefault(curSum - targetSum, 0);
        count2 = count2 + curCount;
