...
public class Knapsack {
    public static void main(String[] args) {
        ...
        int[] weights = {4, 1, 2, 3, 2};
        int[] values = {6, 4, 6, 12, 7};
        int bagWeight = 6;
    ...
    /**
     * 01背包正好装满的最大价值(华为面试题)
     * dp[i][j]：前i件物品在容量为j正好装满的情况下的最大价值
    ...
    /**
     * 01背包正好装满的最小价值
     * dp[i][j]：前i件物品在容量为j正好装满的情况下的最小价值
        ...
        for (int i = 1; i <= listWeights.size(); i++) {
            for (int j = 1; j <= bagWeight; j++) {
                if (listWeights.get(i - 1) > j) {
                    ...
                    dp[i][j] = Math.max(dp[i - 1][j], dp[i - 1][j - listWeights.get(i - 1)] + listValues.get(i - 1));
                }
            }
        }
