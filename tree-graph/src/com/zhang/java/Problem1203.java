...
public class Problem1203 {
    ...
    /**
     * dfs拓扑排序
     * 核心思想：两次拓扑排序，先组间节点拓扑排序，确定小组节点的先后顺序，保证同一小组内节点排序后相邻，
     * 再组内节点拓扑排序，确定组内节点的先后顺序，组内拓扑排序按照组间拓扑排序扩展，得到最终结果
     * 1、对不属于任何小组的节点，即copyGroup[i]为-1的节点分配小组编号，保证这些不属于任何小组的每个节点单独属于一个小组
     * 2、根据图中节点的依赖关系，建立组间图和组内图，如果存在节点u到节点v的边，则组内图中存在节点u到节点v的边，
     * 如果存在节点u到节点v的边，并且两个节点属于不同小组，则组间图中存在节点u所属小组节点到节点v所属小组节点的边
     * 3、先组间节点拓扑排序，确定小组节点的先后顺序，保证同一小组内节点排序后相邻
     * 4、再组内节点拓扑排序，确定组内节点的先后顺序
     * 5、建立小组节点和组内拓扑排序中属于当前小组节点的组内节点集合映射
     * 6、根据映射关系将组间拓扑排序扩展，得到最终结果
    ...
    public int[] sortItems(int n, int m, int[] group, List<List<Integer>> beforeItems) {
        ...
        //2、根据图中节点的依赖关系，建立组间图和组内图，如果存在组内节点u到组内节点v的边，则组内图中存在组内节点u到组内节点v的边，
        //如果存在组内节点u到组内节点v的边，并且两个节点属于不同小组，则组间图中存在组内节点u所属小组节点到组内节点v所属小组节点的边
        ...
        //组间节点拓扑排序访问数组，0-未访问，1-正在访问，2-已访问
        ...
        //6、根据映射关系将组间拓扑排序扩展，得到最终结果
        for (int i = 0; i < groupCount; i++) {
            //不能写为List<Integer> list = map.getOrDefault(groupResult[i]);
            //避免当前小组节点中不存在对应组内节点时，获取空list，导致list.size()空指针异常
            List<Integer> list = map.getOrDefault(groupResult[i], new ArrayList<>());
