...
//        String preorder = "9,3,4,#,#,1,#,#,2,#,6,#,#";
    ...
    public boolean isValidSerialization2(String preorder) {
        ...
        for (int i = 0; i < arr.length; i++) {
            //当前前序遍历过程中出度和入度之差为0，则不是正确的二叉树的前序序列化，返回false
            if (degreeDiff == 0) {
                return false;
            }

            //当前节点为空节点，入度为1，出度为0，即出度和入度之差减1
            if ("#".equals(arr[i])) {
                degreeDiff--;
            } else {
                //当前节点为非空节点，入度为1，出度为2(初始化degreeDiff为1已经考虑根节点入度为0)，即出度和入度之差加1
                degreeDiff++;
            }
        }

        //遍历结束时，如果出度和入度之差等于0，则是是正确的二叉树的前序序列化，返回true
        return degreeDiff == 0;
    }
}
