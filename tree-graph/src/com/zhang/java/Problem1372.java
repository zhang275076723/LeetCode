...
//        String[] data = {"1", "2", "3", "null", "4", "null", "null", "5", "6", "null", "7"};
    ...
    /**
     * dfs
     * 计算当前节点父节点是否向左子节点移动到当前节点，当前节点作为根节点的树对父节点的最长交错路径，更新二叉树的最长交错路径，
     * 返回当前节点的树对父节点的最长交错路径，用于计算父节点作为根节点的树对父节点的最长交错路径
     * 时间复杂度O(n)，空间复杂度O(n)
     *
     * @param root
     * @return
    ...
    public int longestZigZag(TreeNode root) {
        ...
        //注意：只需要1次dfs即可，因为dfs返回值是向父节点的贡献值，而根节点没有父节点，所以不用考虑
        dfs(root, true);
    ...
    /**
     * 返回root的父节点是否向左子节点移动到root，root作为根节点的树对父节点的最长交错路径
     *
     * @param root
     * @param isLeft
     * @return
    ...
    private int dfs(TreeNode root, boolean isLeft) {
        ...
        //当前节点向左子节点移动，左子节点作为根节点的树对父节点的最长交错路径
        int leftMax = dfs(root.left, true);
        //当前节点向右子节点移动，右子节点作为根节点的树对父节点的最长交错路径
        int rightMax = dfs(root.right, false);

        //更新max
        max = Math.max(max, Math.max(leftMax, rightMax));

        //当前节点的父节点向左子树移动到当前节点，则当前节点只能向右子节点移动
        if (isLeft) {
            return rightMax + 1;
        } else {
            //当前节点的父节点向右子树移动到当前节点，则当前节点只能向左子节点移动
            return leftMax + 1;
        }
    }
