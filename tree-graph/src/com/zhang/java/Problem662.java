 ...
 * @Date 2022/7/4 8:12
 * @Author zsy
 * @Description 二叉树最大宽度 字节面试题 类比Problem199、Problem513、Problem515、Problem637、Problem993、Problem1161、Problem1302、Problem2583、Problem2641、Problem3157
...
public class Problem662 {
    ...
    /**
     * dfs
     * 前序遍历每层中第一个访问到的节点即为当前层最左边节点，记录每层最左边节点的下标索引，
     * 计算每层中节点和当前层最左边节点的距离，得到最大宽度
     * 时间复杂度O(n)，空间复杂度O(n)
     *
     * @param root
     * @return
    ...
    private void dfs(TreeNode root, List<Integer> list, int level, int index) {
        ...
        //当前层数等于list大小，则当前节点是当前层中第一个访问到的节点，即为当前层最左边节点，
        //将每层最左边节点的下标索引加入list中，用于更新最大宽度
        if (level == list.size()) {
            list.add(index);
        }
        ...
        dfs(root.left, list, level + 1, index * 2 + 1);
        dfs(root.right, list, level + 1, index * 2 + 2);
    }
