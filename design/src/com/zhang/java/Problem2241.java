...
/**
 * @Date 2024/9/19 08:47
 * @Author zsy
 * @Description 设计一个 ATM 机器 类比Problem2043、Problem2224
...
public class Problem2241 {
    ...
    static class ATM {
        ...
        public ATM() {
            cashArr = new int[]{20, 50, 100, 200, 500};
            countArr = new long[5];
        }

        public void deposit(int[] banknotesCount) {
            for (int i = 0; i < banknotesCount.length; i++) {
                countArr[i] = countArr[i] + banknotesCount[i];
            }
        }
        ...
        public int[] withdraw(int amount) {
            ...
            //优先取较大面值的钞票
            for (int i = cashArr.length - 1; i >= 0; i--) {
                //当前面值cashArr[i]最多只能取countArr[i]张
                result[i] = (int) Math.min(amount / cashArr[i], countArr[i]);
