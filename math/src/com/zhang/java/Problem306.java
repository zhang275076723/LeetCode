...
public class Problem306 {
    ...
    /**
     * 模拟
     * 当前累加序列由前两个累加序列构成，枚举第一个和第二个累加序列，判断能否构成num
     * 时间复杂度O(n^3)，空间复杂度O(n)
     *
     * @param num
     * @return
    ...
    private boolean isValid(String num, String num1, String num2) {
        ...
        while (index < num.length()) {
            ...
            //num字符串中和nextNum相同长度的下一个累加序列长度超过num范围，则num1和num2无法构成num，返回false
            if (index + nextNum1.length() > num.length()) {
                return false;
            }

            //num字符串中和nextNum相同长度的下一个累加序列
            String nextNum2 = num.substring(index, index + nextNum1.length());

            //num1和num2相加得到的下一个累加序列nextNum1和num字符串中和nextNum相同长度的下一个累加序列nextNum2不相等，
            //则num1和num2无法构成num，返回false
