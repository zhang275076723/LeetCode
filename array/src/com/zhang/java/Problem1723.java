 ...
 * @Author zsy
 * @Description 完成所有工作的最短时间 美团机试题 美团面试题 状态压缩类比 二分查找类比Problem4、Problem287、Problem378、Problem410、Problem644、Problem658、Problem1201、Problem1482、Problem2498、CutWood、FindMaxArrayMinAfterKMinus
...
public class Problem1723 {
    ...
    public int minimumTimeRequired(int[] jobs, int k) {
        backtrack(0, 0, new int[k], jobs, k);
    ...
    public int minimumTimeRequired2(int[] jobs, int k) {
        ...
        while (left < right) {
            ...
            if (!backtrack(0, new int[k], jobs, k, mid)) {
                left = mid + 1;
            } else {
    ...
    /**
     * 动态规划+二进制状态压缩
     * dp[i][j]：前i+1个工人分配任务的二进制访问状态j的情况下，完成任务需要的最小时间
     ...
     * @param worker 每个工人分配任务完成需要的时间
     * @param jobs
     * @param k
     */
    private void backtrack(int t, int time, int[] worker, int[] jobs, int k) {
        if (t == jobs.length) {
        ...
        for (int i = 0; i < k; i++) {
            //工人i执行任务jobs[t]之后的执行时间大于等于minTime，则工人i执行任务jobs[t]的最小时间肯定不比minTime小，
            //剪枝，直接进行下次循环
            ...
            backtrack(t + 1, Math.max(time, worker[i]), worker, jobs, k);
    ...
    /**
     * k个工人完成任务需要的最小时间是否超过limit，超过limit，返回false；不超过limit，返回true
     *
     * @param t
     * @param worker
     * @param jobs
     ...
     * @return
     */
    private boolean backtrack(int t, int[] worker, int[] jobs, int k, int limit) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (backtrack(t + 1, worker, jobs, k, limit)) {
                return true;
            }
