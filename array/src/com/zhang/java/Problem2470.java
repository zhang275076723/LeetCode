 ...
 * @Author zsy
 * @Description 最小公倍数为 K 的子数组数目 类比Problem898、Problem2411、Problem2447  最大公因数和最小公倍数类比
...
public class Problem2470 {
    ...
    /**
     * 动态规划
     * arr[0]：从前往后遍历到nums[i]，nums[j]-nums[i]的最小公倍数 (arr[1] <= j <= arr[2])
     * arr[1]：nums[j]-nums[i]的最小公倍数为arr[0]的第一个下标索引j
     * arr[2]：nums[j]-nums[i]的最小公倍数为arr[0]的最后一个下标索引j
     * 遍历到nums[i]，从后往前遍历list中数组，arr[0]和nums[i]求最小公倍数，此时arr[0]表示nums[j]-nums[i-1]的最小公倍数(arr[1] <= j <= arr[2])
     * 如果arr[0] == lcm(arr[0],nums[i])，则说明nums[j]-nums[i-1](arr[1] <= j <= arr[2])和nums[i]的最小公倍数仍为arr[0]，
     ...
     * 否则，更新arr[0]，此时arr[0]表示nums[j]-nums[i]的最小公倍数(arr[1] <= j <= arr[2])，统计以nums[i]结尾子数组的最小公倍数为k的个数
    ...
    public int subarrayLCM2(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            //从后往前遍历list中数组，arr[0]和nums[i]求最小公倍数，此时arr[0]表示nums[j]-nums[i-1]的最小公倍数(arr[1] <= j <= arr[2])
            for (int j = size - 1; j >= 0; j--) {
                ...
                //更新arr[0]，此时arr[0]表示nums[j]-nums[i]的最小公倍数(arr[1] <= j <= arr[2])
                arr[0] = lcm(arr[0], nums[i]);
