...
public class Problem134 {
    ...
    /**
     * 贪心
     * 下标索引i作为起点无法到达下标索引j，则i到j之间的任意下标索引k作为起点都无法到达j，下次要从i+1作为起点继续判断
     * (因为从i开始时，有剩余汽油的情况下都无法到达j，则i到j之间任意下标索引k开始，更不可能到达j)
     * 时间复杂度O(n)，空间复杂度O(1)
     *
     * @param gas
     * @param cost
    ...
    public int canCompleteCircuit2(int[] gas, int[] cost) {
        ...
        for (int i = 0; i < gas.length; i++) {
            ...
            //当前遍历到的下标索引j已经小于起始下标索引i，因为是从0开始遍历，此时0-i都无法作为起点，j在0-i之间，
            //则说明所有的起始位置都不能绕环路行驶一周，返回-1
            if (j < i) {
                return -1;
            }

            //从i无法到j，则i到j之间的任意下标索引k作为起点都无法到达j，下一次起点为i+1
            i = j;
        }
