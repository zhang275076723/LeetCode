 ...
 * @Author zsy
 * @Description 按位或最大的最小子数组长度 类比Problem898、Problem1521、Problem2419、Problem2444、Problem2447、Problem2470 位运算类比 子序列和子数组类比
...
public class Problem2411 {
    ...
    /**
     * 动态规划
     * curArr[0]：从前往后遍历到nums[i]，nums[j]-nums[i]或运算结果 (curArr[1] <= j <= curArr[2])
     * curArr[1]：nums[j]-nums[i]或运算结果为curArr[0]的第一个下标索引j
     * curArr[2]：nums[j]-nums[i]或运算结果为curArr[0]的最后一个下标索引j
     * 遍历到nums[i]，从后往前遍历list中数组，curArr[0]和nums[i]求或运算结果，此时curArr[0]表示nums[j]-nums[i-1]或运算结果(curArr[1] <= j <= curArr[2])
     * 如果curArr[0] == curArr[0]|num[i]，则说明nums[i]表示的二进制数中1，都在nums[j]-nums[i-1](curArr[1] <= j <= curArr[2])或运算结果中出现过，
     * 则nums[0]-nums[i-1]、nums[1]-nums[i-1]...nums[j]-nums[i-1]和nums[i]或运算结果都不会变，
     * 因为随着数组长度变大，或运算结果只增不减，直接跳出循环，遍历下一个nums[i]；
    ...
    public int[] smallestSubarrays3(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            //从后往前遍历list中数组，curArr[0]和nums[i]求或运算结果，此时curArr[0]表示nums[j]-nums[i-1]或运算结果(curArr[1] <= j <= curArr[2])
            for (int j = size - 1; j >= 0; j--) {
                ...
                if (curArr[0] == (curArr[0] | nums[i])) {
                    //如果当前数组或运算结果curArr[0]和前一个数组或运算结果preArr[0]相等，则合并两个arr
                ...
                //更新curArr[0]，此时curArr[0]表示nums[j]-nums[i]或运算结果(curArr[1] <= j <= curArr[2])
                curArr[0] = curArr[0] | nums[i];

                //更新以nums[k]起始的最大或运算结果的最小子数组长度(curArr[1] <= k <= curArr[2])
                for (int k = curArr[1]; k <= curArr[2]; k++) {
                    result[k] = i - k + 1;
                }
