 ...
 * @Author zsy
 * @Description 最大公因数等于 K 的子数组数目 类比Problem898、Problem1521、Problem2411、Problem2419、Problem2444、Problem2470 最大公因数和最小公倍数类比
...
public class Problem2447 {
    ...
    /**
     * 动态规划
     * curArr[0]：从前往后遍历到nums[i]，nums[j]-nums[i]的最大公因数 (curArr[1] <= j <= curArr[2])
     * curArr[1]：nums[j]-nums[i]的最大公因数为curArr[0]的第一个下标索引j
     * curArr[2]：nums[j]-nums[i]的最大公因数为curArr[0]的最后一个下标索引j
     * 遍历到nums[i]，从后往前遍历list中数组，curArr[0]和nums[i]求最大公因数，此时curArr[0]表示nums[j]-nums[i-1]的最大公因数(curArr[1] <= j <= curArr[2])
     * 如果curArr[0] == gcd(curArr[0],nums[i])，则说明nums[j]-nums[i-1](curArr[1] <= j <= curArr[2])和nums[i]的最大公因数仍为curArr[0]，
    ...
    public int subarrayGCD2(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            //从后往前遍历list中数组，curArr[0]和nums[i]求最大公因数，此时curArr[0]表示nums[j]-nums[i-1]的最大公因数(curArr[1] <= j <= curArr[2])
            for (int j = size - 1; j >= 0; j--) {
                ...
                //则nums[0]-nums[i-1]、nums[1]-nums[i-1]...nums[j]-nums[i-1]和nums[i]的最大公因数都不会变，
                //因为随着数组长度变大，最大公因数只减不增，直接跳出循环，统计以nums[i]结尾子数组的最大公因数为k的个数，遍历下一个nums[i]
                if (curArr[0] == gcd(curArr[0], nums[i])) {
                    //如果当前数组的最大公因数curArr[0]和前一个数组的最大公因数preArr[0]相等，则合并两个arr
                ...
                //更新curArr[0]，此时curArr[0]表示nums[j]-nums[i]的最大公因数(curArr[1] <= j <= curArr[2])
                curArr[0] = gcd(curArr[0], nums[i]);
